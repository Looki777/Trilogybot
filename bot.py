...
def get_peak_online():
    conn = sqlite3.connect("bot_stats.db", timeout=5)
    cursor = conn.cursor()
    cursor.execute("SELECT peak_online, peak_date FROM server_stats WHERE id = 1")
    row = cursor.fetchone()
    conn.close()
    return (row[0], row[1]) if row else (0, "")

def update_peak_online(current_players):
    peak, peak_date = get_peak_online()
    if current_players > peak:
        conn = sqlite3.connect("bot_stats.db", timeout=5)
        cursor = conn.cursor()
        now = time.strftime("%d.%m.%Y %H:%M")
        cursor.execute("UPDATE server_stats SET peak_online = ?, peak_date = ? WHERE id = 1", (current_players, now))
        conn.commit()
        conn.close()
...
def check_samp_server():
    ...
    t.start()
    t.join(timeout=10)  # максимум 10 сек на ответ сервера
    ...
    return (
        ...
        f"🏆 <b>Пик онлайна:</b> {peak} <i>(был {peak_date})</i>\n"
        f"⚡ <b>Пинг:</b> {ping} мс\n\n"
        f"🟢 Статус: Работает"
    )
